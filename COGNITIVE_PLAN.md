...
2. **Retrieval quality (gates Phase 3 vectors).** 50 query→expected-fact pairs; lexical baseline vs hybrid; ship vectors only at ≥ 15% recall@5 improvement; negative result documented otherwise.
   - **P4.3 — upgrade path after the negative result (2026-09).** The 50-pair eval recorded lexical recall@5 = 0.980 vs hybrid = 0.800 → `RetrievalGate.LOCAL_BRANCH_SHIPS = false`. The hybrid machinery stays in the tree; it may be re-enabled ONLY when ALL of the following hold, re-measured through `RetrievalEvalTest` + `EmbedderBenchmark`:
     1. **Real-transcript corpus.** The 50-pair set is synthetic. Collect ≥ 200 query→fact pairs from real device transcripts (≥ 2 weeks of daily use). Gate: lexical recall@5 < 0.90 on THAT set (i.e. the corpus contains queries lexical actually fails on — otherwise the comparison is meaningless) AND hybrid beats lexical by ≥ 15% relative recall@5 on it.
     2. **Held-out re-tuning.** RRF weights re-tuned on a held-out split (never the eval set); lexical recall@5 regression ≤ 2%; per-query deltas logged by the benchmark harness.
     3. **Latency budget.** Embedder end-to-end ≤ 250 ms on the Kirin 710A target (plan §8 budget) measured on device.
     The flip is: `RetrievalGate.LOCAL_BRANCH_SHIPS = true` + CHANGELOG entry + a live-toggle regression test (Phase-3 vectors already ship with one).
