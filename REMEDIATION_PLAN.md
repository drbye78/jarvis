...
| Phase 7 slice 5 (Settings decomposition U1) | **IN PROGRESS** | **Increment 1 (done):** new `Widget.Jarvis.SettingsCard` style in `values/styles.xml` — the identical five appearance attributes (`cardBackgroundColor`/`cardCornerRadius`/`cardElevation`/`contentPadding` ×9 cards) are now defined once; 36 copy-pasted lines removed (1154→1127). `parent=""` on that style is required and load-bearing: AAPT reads the dots in a style name as implicit inheritance and otherwise demands a non-existent `Widget.Jarvis` style (first build failed exactly so). `layout_width`/`layout_height` stay explicit at each call site rather than moving into the style — a zero-sized card is not a risk worth four saved lines, and per-card margins genuinely differ (2 cards add `top 6dp`). **Increment 2 (done):** new pure-JVM `ui/SettingsMapping.kt` owns the view↔preference encodings — `Player` enum + `playerForPref`/`playerPrefFor` (accepts BOTH `ru.yandex.music` and the legacy `com.yandex.music`, canonicalises on write), `nextEmbedder` (now case-insensitive and self-healing: an unknown value restarts at AUTO instead of silently sticking), `followUpSeconds`, `quietHour` (`+23`/`%24` wraparound made explicit and direction-correct), `quota`, `selectedVoiceId`, `isSherpaEngine`; 12 `SettingsActivity` call sites rewired and the now-dead `embedderOrder` local deleted. New `SettingsMappingTest` ×19 including exhaustive loops over all 24 hours and the full quota range. **Increment 3 (done):** `activity_settings.xml` 1127→123 lines — header + 9 `<include>`s; the nine cards live in `settings_card_*.xml` (46–189 lines each). Card sizing moved onto the `<include>` tag, deliberately NOT relying on the include-override rule for a screen with no UI coverage; each include keeps an explicit `layout_width`/`layout_height`. **Guard (done):** new pure-JVM `SettingsLayoutTest` ×3 closes the gap that made this refactor risky in the first place — `R.id` constants are generated from ALL layouts, so a dropped/renamed id still compiles and only NPEs at runtime (and the split itself compiles even when an `<include>` is malformed). It asserts (a) every `R.id.X` in `SettingsActivity.kt` is declared by the host or its include closure, (b) no id is declared twice, (c) every `settings_card_*.xml` on disk is included exactly once with explicit sizing. It immediately earned its keep: it caught the split emitting bare `layout="@layout/…"` instead of `android:layout="@layout/…"` — 9 includes that built cleanly and would have rendered an EMPTY settings screen on device. **Remaining:** extract the nine card sections out of `SettingsActivity` (1243 LOC) into `SettingsSection` subclasses; add the `SettingRow` style if a genuine duplicated row pattern remains. **976 tests, 0 failures, 6 skipped, detekt clean** |

**Known flake (pre-existing, tracked):** `MemoryToolsTest.gather respects the disabled
switch and the budget shape` failed twice in full-suite runs, then passed on 3
consecutive full runs and 6 isolated runs. Hypothesis: the coordinator's own scope is
`Dispatchers.IO` while the test asserts immediately after a fire-and-forget write, so
the assertion can race under load. Not reproduced on demand; fix when it next appears.
...
- **Porcupine:** `libpv_porcupine.so` 3.0.0 is aligned at `0x1000` (not 16 KB
  compliant); Sherpa-ONNX and ONNX Runtime are already `0x4000`. Upgrade to Porcupine
  4.x (Java API is a strict superset of the symbols used; 3.0.2 is the first compliant
  3.x release, 4.0.2 the latest). Play blocks non-compliant updates from Feb 1 2027.
- Re-verify with `readelf -l` that every packaged `.so` has LOAD align `0x4000`.
- **Exit:** gate green; `aapt dump badging` shows targetSdk 36; ELF check passes.
...
| 9 | `aapt dump badging` targetSdk; `readelf -l` align `0x4000` for all `.so` |
