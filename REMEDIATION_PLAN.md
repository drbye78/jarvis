...
| N7 cloud-vector purge (privacy closure) | **DONE** | `memory.cloudEnabled=false` previously only *refused to read* CLOUD vectors — the embeddings of user facts stayed in Room indefinitely. Now `CognitiveCoordinator.purgeCloudVectors()` deletes every engine space `!= EmbeddingEngine.LOCAL_ID` (so stale/renamed cloud ids are caught too), reached from two entry points: a reactive `startCloudPurgeWatch()` (`cloudEnabled.drop(1).filter { !it }`, launched on the coordinator's supervised scope, wired in `AppGraph.start()`) purges the instant the switch flips false, and `vectorMaintenance()` purges as its FIRST statement (before the `metaDao` early return) so a disable that happened while the app was killed is still reconciled the next night. The two DAO methods (`distinctEngineIds()`/`deleteForEngine()`) that previously had zero production callers now have real ones. Content-free INFO log (space + row counts only). New `CloudVectorPurgeTest` (2 tests: live flip keeps LOCAL / drops CLOUD; cold-start `onMaintenance()` backstop) — both fail without the fix. Detekt's class-level function cap was reached (25/25) and resolved with `@Suppress("TooManyFunctions")` on `CognitiveCoordinator`, the same precedent as `VectorDaos`/`BehaviorDaos` and the class's own "the ONE class the rest of the app sees" KDoc. **987 tests, 0 failures, 6 skipped, detekt clean** |
...
**N7 resolved (all audit findings now closed or explicitly accepted).** Turning
`memory.cloudEnabled` off now deletes every CLOUD vector space — immediately via the
reactive watch, and again as a nightly `vectorMaintenance()` backstop for a disable that
happened while the app was killed. Gate green (**987 tests** — 985 + 2 — 0 failures,
6 skipped, detekt 0 findings; the added class-level `TooManyFunctions` suppression on
`CognitiveCoordinator` follows the existing `VectorDaos`/`BehaviorDaos` precedent and the
class's own "the ONE class the rest of the app sees" KDoc).
...
- **N7 (Medium, privacy) — RESOLVED:** `memory.cloudEnabled=false` now purges CLOUD
  vectors instead of only refusing to read them. The previously-unwired DAO seam
  (`VectorDaos.distinctEngineIds()`/`deleteForEngine()`) gained real callers:
  `CognitiveCoordinator.startCloudPurgeWatch()` (wired in `AppGraph.start()`) deletes
  every engine space that is not the on-device `LOCAL_ID` the moment the switch flips
  false, and the nightly `vectorMaintenance()` backstop covers a disable that landed
  while the app was killed. Deleting every non-LOCAL space also catches stale/renamed
  cloud ids. Content-free INFO log (space/row counts only). Pinned by
  `CloudVectorPurgeTest` (2 tests: live flip + cold-start backstop).
...
- **Dependency bumps:** protobuf-java 3.25.3 → 4.x or at minimum a patched 3.25.x
  ...
  > **Resolved (as-built): declared `protobuf` + `protoc` 3.25.3 → 3.25.9, NOT 4.x.**
  > The declared 3.25.3 was never the version in use — grpc-protobuf 1.83.1 already
  > forces the runtime to 3.25.9, which is above the CVE-2024-7254 floor (3.25.5), so
  > the real defect was the stale pin, not an active vulnerability. grpc-java 1.83.1
  > pins protobuf 3.25.9 itself and explicitly refuses 4.x ("not yet ABI compatible",
  > protobuf#17247; grpc-java#11015 tracks it), and deps.dev reports zero advisories
  > for 3.25.9 — so 3.25.9 is the correct, *supported* alignment and 4.36.2 would have
  > been an unsupported combination. `javax.annotation:javax.annotation-api` was also
  > dropped: grpc-java ≥1.74.0 generates stubs with `@generated=omit` (the app is on
  > 1.83.1), confirmed by zero `javax.annotation` hits in all 82 generated sources.
  > Revisit the 4.x move only when grpc-java ships it, and move the whole set together.
- **CI:** pin GitHub Actions to commit SHAs instead of tags.
  > **Resolved (as-built):** all 25 `uses:` lines pinned to 40-hex SHAs, annotated
  > tags dereferenced, tag retained as a trailing comment, independently reproduced.
- **Doc drift:** `README.md:5,119`, `AGENTS.md:8,47`, `RUNBOOK.md:3`,
  ...
  > **Resolved (as-built):** aligned; the plan-flagged `README.md:5,119` and
  > `AGENTS.md:47` were already correct (0.2.2 / v2) and were left alone. `AGENTS.md`
  > also gained the mandated Phase 1–3 invariants.
...
| N7 | `CloudVectorPurgeTest` seeds LOCAL+CLOUD rows, flips `cloudEnabled` false (and, separately, runs `onMaintenance()` from a cold `cloudEnabled=false`) and asserts the CLOUD space is gone while the LOCAL row is untouched — both fail on pre-fix code (nothing else deletes cloud rows, and `vectorMaintenance()` returned early while cloud was off) |
