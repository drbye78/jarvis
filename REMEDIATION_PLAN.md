...
| Phase 7 slice 5 (Settings decomposition U1) | **IN PROGRESS** | **Increment 1 (done):** new `Widget.Jarvis.SettingsCard` style in `values/styles.xml` — the identical five appearance attributes (`cardBackgroundColor`/`cardCornerRadius`/`cardElevation`/`contentPadding` ×9 cards) are now defined once; 36 copy-pasted lines removed from `activity_settings.xml` (1154→1127). `parent=""` on that style is required and load-bearing: AAPT reads the dots in a style name as implicit inheritance and otherwise demands a non-existent `Widget.Jarvis` style (first build failed exactly so). `layout_width`/`layout_height` were deliberately left explicit at each call site rather than moved into the style — the build cannot verify rendering, and a zero-sized card is not a risk worth taking for four saved lines. Per-card margins also stay explicit (they legitimately differ: 2 cards add `top 6dp`). **Remaining:** split the layout into header + 9 includes, add the `SettingRow` style, and extract the nine card sections out of `SettingsActivity` (1243 LOC) into `SettingsSection` subclasses; then gate. **954 tests, 0 failures, 6 skipped, detekt clean** |
